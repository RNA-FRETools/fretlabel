...
def couple_dye2baselinker(
    ...
    try:
        ...
        align_dye_atoms = [
            at["name"]
            for at in cmd_gui.get_model("neighbor name {}".format(carbonylC_dye), 0)[
                "atom"
            ]
        ]
        ...
        cmd_gui.pair_fit(
            *[
                ...
                for i in [
                    [
                        "{} and name {}".format(dye, a1),
                        "{} and name {}".format(baselinker, a2),
                    ]
        ...
        cmd_gui.remove(
            "{} and name {}".format(
                baselinker, "+".join(str(i) for i in remove_baselinker_atoms)
            )
        )
        cmd_gui.create(
            "{}_{}".format(dye, baselinker), "{} or {}".format(dye, baselinker)
        )
...
def write_rtp(filename, molecule):
    ...
    1       1          9          4        1         3      1     0\n\n"""
        ...
        for mol in molecule:
            ...
            f.write("[ atoms ]\n")
            f.write(
                mol.atoms[["atom", "type", "charge", "nr"]].to_string(
            ...
            f.write("\n[ impropers ]\n")
            f.write(
                mol.impropers[["ai", "aj", "ak", "al"]].to_string(
                    header=False, index=False
                )
            )
            f.write("\n\n")
    ...
    try:
        ...
        if os.path.isfile(filename) and not overwrite:
            ...
            with open(filename, "w") as f:
                ...
                f.write(
                    "{:d} {:d} {:d}\n".format(
                        len(model["atom"]), len(model["bond"]), len(visited_res)
                    )
                )
                f.write("SMALL\nUSER_CHARGES\n@<TRIPOS>ATOM\n")
                visited_res = []
                ...
                for i, bond in enumerate(model["bond"]):
                    f.write(
                        "{:d} {:d} {:d} {:d}\n".format(
                            i + 1, *np.array(bond["index"]) + 1, bond["order"]
                        )
                    )
...
def pandasMol2_replace(
    from_df, to_df, replace_name, subst_value=None, match_name="atom_name"
):
    """
...
def update_specbond(
    ...
    try:
        specbond_newline = pd.DataFrame(
            [specbond_string.split()], columns=specbond_format
        )
    except ValueError:
        print(
        ...
        if os.path.isfile(outputfile) and not overwrite:
            print(
                "File already exists, do not overwrite. Either change outputfilename or set overwrite=True"
            )
        else:
...
def update_residuetypes(
    ...
    residuetypes_df = pd.read_csv(
        inputfile, sep="\s+", names=residuetype_format, na_filter=False
    )

    try:
        residuetype_newline = pd.DataFrame(
            [residuetypes_string.split()], columns=residuetype_format
        )
    except ValueError:
        print(
        ...
        residuetypes_df = residuetypes_df.append(residuetype_newline).reset_index(
            drop=True
        )
...
class Parameters:
    ...
    def read_frcmod(cls, filename, atomtypes_molecule=None):
        ...
        if (atomtypes_molecule is not None) and atomtype_list:
            atomtypes = atomtypes_molecule[
                atomtypes_molecule["name"].isin(atomtype_list)
            ]
        else:
            atomtypes = None

        bondtypes = pd.DataFrame(
            bondtype_list, columns=["i", "j", "funct", "b0", "kb", "comment"]
        )
        angletypes = pd.DataFrame(
            angletype_list, columns=["i", "j", "k", "funct", "th0", "cth", "comment"]
        )
        propertypes = pd.DataFrame(
            propertype_list,
    ...
    @classmethod
    def read_specialbond(
        cls, amberdyes, atoms_amberdyes, atoms_other, ff_name="AMBER-DYES"
    ):
        """
        ...
        if "bondtypes" in atoms_amberdyes.keys():
            for a, b in zip(atoms_amberdyes["bondtypes"], atoms_other["bondtypes"]):
                bondtype1 = copy.deepcopy(
                    amberdyes.bondtypes[(amberdyes.bondtypes[["i", "j"]] == a).all(1)]
                )
                bondtype2 = copy.deepcopy(
                    amberdyes.bondtypes[
                        (amberdyes.bondtypes[["i", "j"]] == a[::-1]).all(1)
                    ]
                )
                ...
                if not bondtype2.empty:
                    bondtype2[["i", "j"]] = b[::-1]
                    bondtype2["comment"] = "; same as {} {}-{}".format(
                        ff_name, *a[::-1]
                    )
        ...
        if "angletypes" in atoms_amberdyes.keys():
            for a, b in zip(atoms_amberdyes["angletypes"], atoms_other["angletypes"]):
                angletype1 = copy.deepcopy(
                    amberdyes.angletypes[
                        (amberdyes.angletypes[["i", "j", "k"]] == a).all(1)
                    ]
                )
                angletype2 = copy.deepcopy(
                    amberdyes.angletypes[
                        (amberdyes.angletypes[["i", "j", "k"]] == a[::-1]).all(1)
                    ]
                )
                ...
                if not angletype2.empty:
                    ...
                    angletype2["comment"] = "; same as {} {}-{}-{}".format(
                        ff_name, *a[::-1]
                    )
        ...
        if "propertypes" in atoms_amberdyes.keys():
            for a, b in zip(atoms_amberdyes["propertypes"], atoms_other["propertypes"]):
                propertype1 = copy.deepcopy(
                    amberdyes.propertypes[
                        (amberdyes.propertypes[["i", "j", "k", "l"]] == a).all(1)
                    ]
                )
                propertype2 = copy.deepcopy(
                    amberdyes.propertypes[
                        (amberdyes.propertypes[["i", "j", "k", "l"]] == a[::-1]).all(1)
                    ]
                )
                if not propertype1.empty:
                    propertype1[["i", "j", "k", "l"]] = b
                    propertype1["comment"] = "; same as {} {}-{}-{}-{}".format(
                        ff_name, *a
                    )
                    propertype_list.append(propertype1)
                if not propertype2.empty:
                    propertype2[["i", "j", "k", "l"]] = b[::-1]
                    propertype2["comment"] = "; same as {} {}-{}-{}-{}".format(
                        ff_name, *a[::-1]
                    )
        ...
        if "impropertypes" in atoms_amberdyes.keys():
            for a, b in zip(
                atoms_amberdyes["impropertypes"], atoms_other["impropertypes"]
            ):
                impropertype1 = copy.deepcopy(
                    amberdyes.impropertypes[
                        (amberdyes.impropertypes[["i", "j", "k", "l"]] == a).all(1)
                    ]
                )
                impropertype2 = copy.deepcopy(
                    amberdyes.impropertypes[
                        (amberdyes.impropertypes[["i", "j", "k", "l"]] == a[::-1]).all(
                            1
                        )
                    ]
                )
    ...
    def append(self, parameters):
        if self.atomtypes is not None:
            self.atomtypes = self.atomtypes.append(
                parameters.atomtypes
            ).drop_duplicates()
        if self.bondtypes is not None:
            self.bondtypes = self.bondtypes.append(
                parameters.bondtypes
            ).drop_duplicates()
        if self.constrainttypes is not None:
            self.constrainttypes = self.constrainttypes.append(
                parameters.constrainttypes
            ).drop_duplicates()
        if self.angletypes is not None:
            self.angletypes = self.angletypes.append(
                parameters.angletypes
            ).drop_duplicates()
        if self.propertypes is not None:
            self.propertypes = self.propertypes.append(
                parameters.propertypes
            ).drop_duplicates()
        if self.impropertypes is not None:
            self.impropertypes = self.impropertypes.append(
                parameters.impropertypes
            ).drop_duplicates()
    ...
    def add2ff(self, ff_folder, outputdir="./"):
        ...
        """
        for filename in [
            "{}/ffnonbonded.itp".format(ff_folder),
            "{}/ffbonded.itp".format(ff_folder),
        ]:
            with open(filename, "r") as f:
                lines = f.readlines()
                keynames = []
    ...
    def write_atp(self, filename):
        ...
        with open(filename, "w") as f:
            f.write(
                self.atomtypes[["name", "mass"]].to_string(header=False, index=False)
            )
...
class Molecule:
    ...
    def read_molecule(cls, filename, comment=None):
        ...
        return cls(
            moleculetype, atoms, bonds, angles, propers, impropers, atomtypes_molecule
        )
    ...
    def remove_atom(self, name):
        ...
        if self.atoms is not None:
            self.atoms = self.atoms.drop(
                self.atoms[(self.atoms["atom"] == name)].index
            ).reset_index(drop=True)
            self.atoms["nr"] = range(self.atoms.shape[0])
        if self.bonds is not None:
            self.bonds = self.bonds.drop(
                self.bonds[
                    (self.bonds["ai"] == name) | (self.bonds["aj"] == name)
                ].index
            ).reset_index(drop=True)
