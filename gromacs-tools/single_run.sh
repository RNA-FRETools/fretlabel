...
Usage: single_run.sh -c <structure file (.gro)> -d <mdp directory>" 1>&2; exit 1; }
...
mkdir nvt
gmx grompp -f "$mdp_dir"/nvt.mdp -c em/"$structureName".gro -r em/"$structureName".gro -p "$structureName".top -o nvt/"$structureName".tpr -po nvt/"$structureName".mdp || { echo "Error: grompp for nvt failed" ; cleanup nvt ; exit 1; }
