...
if [ -z "$timetoextend" ]; then
    ...
    gmx mdrun -v -s md0/"$inputName".tpr -cpi md0/"$inputName".cpt -x md0/"$inputName".xtc -e md0/"$inputName".edr -g md0/"$inputName".log -append || { echo "-> Error: gmx mdrun for md0 restart failed" ; cleanup md0 ; exit 1; }
else
    # extend finished run
